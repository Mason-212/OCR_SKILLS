...
A dense handwritten page takes **about 2–6 minutes** (local glm-ocr per region, retries on errors). Quality first — it will not silently fall back to the fast Apple Vision engine if glm-ocr is installed. Same quality path as before; the Mac work is lighter (compiled prep, no extra shrink/house-style passes, watcher sleeps when inboxes are unchanged).
...
3. Boost contrast/sharpen for handwriting (local Core Image).
4. Detect 1- vs 2-column / split-down-the-middle layout and boxed frames with on-device Apple Vision; crop those regions separately.
5. Detect underlined phrases from the ink on the page (local pixel check).
6. Run local **Ollama `glm-ocr`** per region (layout + emphasis-aware prompts). Finish words eaten by a hole punch / binder ring from the rest of the sentence; do not invent new facts.
7. Stitch: columns as a full-width notebook fold (not skinny cards); drawn boxes as `> [!box]`.
8. **House style on every page** (this is the default for all future scans): blank line between paragraphs; key ideas/titles like *The Age of Enlightenment* as `### <u>**Title**</u>`; hand underlines as `<u>…</u>`; questions bold; long quotes as blockquotes; spelling from the page + Mac dictionary (e.g. influence).
9. Optional local `llama3.2:3b` polish, then house style runs again so polish cannot strip spacing or underlines.
